...
export const TechModule = {
    ...
    init(panel) {
        this.img = panel.querySelector('img') || document.getElementById(`img-${this.id}`);
    ...
    onFrame(frame) {
        ...
        // Pause iken küçük buffer tut ki resume anında hızlı başlasın.
        if (!this.isPlaying) {
    ...
    _presentHeadFrameBeforeAudio() {
        ...
        try {
            ...
            try {
                this.img.src = this.canvas.toDataURL('image/jpeg', 0.82);
            } catch (e) {
                log('warn', this.id, `Ilk MJPEG frame set edilemedi: ${e?.message || e}`);
            }
        } catch (e) {
            log('warn', this.id, `Ilk kare (canvas): ${e?.message || e}`);
        } finally {
                ...
                while (this.frameQueue.length > 0) {
                    ...
                    if (frameTs <= masterTs + WIN) {
                        ...
                        this.canvas.toBlob((blob) => {
                            if (!blob || currentGen !== this.renderGen) return;
