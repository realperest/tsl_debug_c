...
export const TechModule = {
    ...
    init(panel) {
        this.img = panel.querySelector('img');
    ...
    async load(token, info) {
        ...
        try {
            ...
            self.mp4boxfile.onReady = (readyInfo) => {
                ...
                if (!track) { log('error', self.id, 'Track yok.'); return; }

                self.timescale = track.timescale || 90000;
                log('info', self.id, `Track: ${track.codec} | Timescale: ${self.timescale}`);
                ...
                self.decoder = new VideoDecoder({
                    output: (frame) => self.processFrame(frame),
                ...
                try {
                    ...
                    log('error', self.id, `Configure: ${e.message}`);
                }
